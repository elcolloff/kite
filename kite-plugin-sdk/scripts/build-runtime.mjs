...
const getSDKRuntimeEntry = () => {
  const sourceEntry = path.resolve(packageDir, 'src/host-runtime.ts')
  if (existsSync(sourceEntry)) {
    return sourceEntry
  }
  return path.resolve(packageDir, 'dist/host-runtime.js')
}
...
export async function buildPluginRuntime({ outdir, resolveFrom }) {
  ...
  const { build: viteBuild } = await importResolvedModule('vite', moduleResolver)
